...
PluginRuntime::BindNatives(const ke::Ref<INativeRegistry>& aRegistry)
{
  ...
  for (size_t i = 0; i < image_->NumNatives(); i++) {
    registry->Bind(this, &natives_[i]);

    if (!natives_[i].status &&
        !(natives_[i].flags & (SP_NTVFLAG_OPTIONAL|SP_NTVFLAG_EPHEMERAL)))
    {
      all_bound = false;
    }
  }
...
{
  Ref<NativeGroup> group = aGroup->ToNativeGroup();

  for (size_t i = 0; i < image_->NumNatives(); i++) {
    if (natives_[i].group == group)
      natives_[i].flags |= SP_NTVFLAG_OPTIONAL;
  }
}
